...
public class SQLInjectionAnalyzerRunnableTest {
    ...
    public void testRun() throws Exception {
        new File("./build/SQLIAnalyzer/SQLIAnalyzerDiskStorage.xml").delete();
        ...
        try {
            ...
            new Expectations() {
                SQLInjectionAnalyzerEntry entry;
                Logger logger;
                {
                    ...
                    entry = new SQLInjectionAnalyzerEntry("select sysdate from dual", 0);
                    result = new IOException("JUnit");
                    times = 1;
                    ...
                    logger.log(Level.SEVERE, "Failed to store result on disk.", (IOException) any);
                    times = 1;
                }
            };
            runnable2.run();
